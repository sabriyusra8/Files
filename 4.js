TorrentStream.Controls = function(container, conf)
{
    ...
    function ytSetSizeButtonsState(wide) {
        ...
        if(wide === undefined) {
            wide = $("#watch7-container").hasClass("watch-wide");
        }
        
        if(wide) {
            $("#" + _self.id("yt-expand"), _contextDocument).css({'background-position':'-327px -199px','top':'6px'});
            ...
            $("#" + _self.id("yt-squeeze"), _contextDocument).css({'background-position':'-394px -199px'});
        }
    }
    ...
    function attachControlsEvents()
    {
        ...
        if(conf.youtube) {
            ...
            $("#" + _self.id("yt-expand"), _contextDocument).click(function() {
                    $("#player").addClass("watch-medium").addClass("watch-playlist-collapsed");
            ...
            $("#" + _self.id("yt-squeeze"), _contextDocument).click(function() {
                    $("#player").removeClass("watch-medium").removeClass("watch-playlist-collapsed");
                    $("#watch7-container").removeClass("watch-wide");
    ...
    this.setYoutubeFormats = function(formats) {
        ...
        if(formats === null) {
            $("#"+_self.id("playlist-box"), _contextDocument).hide();
            return;
        }
    ...
    /**
    * Display "Play" button with acestream://... link (when plugin is not available).
    *
    * @param string contentId
    */
    this.displayAceStreamLink = function(contentId) {
        var containerHeight = $("#"+_self.id(), _contextDocument).height(),
            containerWidth = $("#"+_self.id(), _contextDocument).width(),
            left = containerWidth / 2 - 64,
            top = containerHeight / 2 - 64;
            
        $("#" + _self.id("content"), _contextDocument).append('<a href="acestream://' + contentId + '" style="color: #fff; font-size: 48px; text-decoration: none; font-weight: bold; position: absolute; left: '+left+'px; top: '+top+'px;"><img src="http://static.acestream.org/magicplayer/logo/128.png" width="128" height="128" /></a>');
        
        if("android" == _platform) {
            $("#" + _self.id("content"), _contextDocument).append(
                '<div style="color: #aaa; position: absolute; top: ' + (top + 128 + 50) + 'px; left: 0px; width: 100%; text-align: center;">'+
                'Ace Stream must be installed to watch this video.<br/>'+
                'If you don\'t have Ace Stream, you can install it <a href="http://dl.acestream.org/products/acestream-full/android/armv7/latest" style="text-decoration: underline; color: #aaa;">here</a>'+
                '</div>'
                );
        }
    };
    ...
    if(conf.style == "internal" && availablePlugin.type != 0 && availablePlugin.type != 2 && availablePlugin.type != 3) {
